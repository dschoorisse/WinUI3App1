...
namespace WinUI3App1
{
    ...
    public class EDSDK
    {
        ...
        private const string DLL_PATH = @"EDSDK\EDSDK.dll";

        // Static constructor to set native DLL search path
        static EDSDK()
        {
            // Set DLL directory to ensure proper loading
            try
            {
                string dllDirectory = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "EDSDK");
                if (Directory.Exists(dllDirectory))
                {
                    // Add the EDSDK directory to the DLL search path
            ...
            {
                Console.WriteLine("Error setting DLL directory: " + ex.Message);
            }
        }

        // Win32 API function to set DLL search directory
